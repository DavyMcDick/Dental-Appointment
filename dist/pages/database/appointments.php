...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  $id = $_POST['user_id']; // ← Get the correct ID from hidden field
  $name = $_POST['fname'];
  $lname = $_POST['lname'];
  $email = $_POST['email'];
...
<div class="d-flex justify-content-between align-items-center mt-3">
    ...
    <div class="flex-grow-1.5">
        <?php if (isset($_GET['message'])): ?>
            <div class="alert 
                <?php
                if ($_GET['message'] == 'success') echo 'alert-success';
                elseif ($_GET['message'] == 'error') echo 'alert-danger';
                elseif ($_GET['message'] == 'invalid') echo 'alert-warning';
                elseif ($_GET['message'] == 'updated') echo 'alert-success';
                elseif ($_GET['message'] == 'update_error') echo 'alert-danger';
                ?>
                mb-0 py-2 px-3">
                <?php
                if ($_GET['message'] == 'success') echo " User deleted successfully!";
                elseif ($_GET['message'] == 'error') echo " Error deleting user.";
                elseif ($_GET['message'] == 'invalid') echo " Invalid user ID.";
                elseif ($_GET['message'] == 'updated') echo " User updated successfully!";
                elseif ($_GET['message'] == 'update_error') echo " Error updating user.";
                ?>
            </div>
        <?php endif; ?>
    </div>
                          ...
                          <tr>
                            ...
                            <td>
                              <a href="#"
                                 ...
                                 data-bs-target="#editAppointmentModal">Edit</a>
                              |
                    ...
                    <div class="d-flex justify-content-end mt-3">
                        <nav aria-label="Page navigation">
                            <ul class="pagination">
                                <?php if ($page > 1): ?>
                                    <li class="page-item">
                                        <a class="page-link" href="appointments.php?page=<?= $page - 1; ?>" aria-label="Previous">
                                ...
                                <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                                    <li class="page-item <?= $i == $page ? 'active' : '' ?>">
                                        <a class="page-link" href="appointments.php?page=<?= $i; ?>">
                                            <?= $i; ?>
                                        </a>
                                    </li>
                                <?php endfor; ?>
...
document.addEventListener("DOMContentLoaded", function() {
    setTimeout(function() {
        ...
        if (alertBox) {
            ...
            setTimeout(() => alertBox.remove(), 500); // Remove it from the DOM
        }
    }, 5000); 
});
</script>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    editButtons.forEach(button => {
        button.addEventListener('click', function () {
            modal.querySelector('input[name="user_id"]').value = this.dataset.id;
            modal.querySelector('input[name="fname"]').value = this.dataset.fname;
            modal.querySelector('input[name="lname"]').value = this.dataset.lname;
            modal.querySelector('input[name="email"]').value = this.dataset.email;
            modal.querySelector('input[name="password"]').value = this.dataset.password;
        });
    });
});
</script>
