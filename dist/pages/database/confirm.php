...
/// === START: Total Confirmed Time for Selected Date ===
date_default_timezone_set('Asia/Manila'); // Always good to set timezone early
...
// If user searched a specific date, use it
$date_filter = isset($_GET['search_date']) && $_GET['search_date'] !== ''
    ? $_GET['search_date']
    : $futureDate; // Default search: today + 2 days

$totalMinutes = 0;

// ✅ Query today's and +2 days confirmed appointments
$timeQuery = "SELECT Time, Dates FROM confirm 
              WHERE Status = 'Confirmed' 
              AND (Dates = '$today' OR Dates = '$futureDate')";

$timeResult = $conn->query($timeQuery);

if ($timeResult && $timeResult->num_rows > 0) {
    while ($timeRow = $timeResult->fetch_assoc()) {
        $minutes = (int) filter_var($timeRow['Time'], FILTER_SANITIZE_NUMBER_INT);
        $totalMinutes += $minutes;
    }
}

$maxMinutesPerDay = 420;
$remainingMinutes = $maxMinutesPerDay - $totalMinutes;
// === END: Total Confirmed Time Logic ===
        ...
        <div class="card">
          <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap">
              <div class="btn btn-primary text-white mb-2 mb-md-0" style="pointer-events: none; cursor: default;">
                  <strong>Total booked time:</strong> <?= $totalMinutes ?> mins /
                  <strong>Remaining:</strong> <?= max(0, $remainingMinutes) ?> mins out of 420
              </div>
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        echo "<tr>
                ...
                <td>" . $row['Dates'] . "</td>
                <td>
                  <a href='move_appointment.php?id=" . $row["ID"] . "&status=Confirmed' 
                     onclick=\"return hideRow(this, 'Confirm this appointment?');\">
                     Confirm
                  </a>
                  | 
                  <a href='move_appointment.php?id=" . $row["ID"] . "&status=Cancelled' 
                     onclick=\"return hideRow(this, 'Cancel this appointment?');\">
                     Cancel
                  </a>
                </td>
              </tr>";
